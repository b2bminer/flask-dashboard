...
def extract_stock_symbol(url):
    """
    สกัดชื่อหุ้นจาก URL ของรูปภาพ
    ตัวอย่างชื่อไฟล์: AAPL_20240101.png, TSLA_daily_chart.jpg
    """
    import re
    # ดึงชื่อไฟล์จาก URL
    ...
    # หาส่วนที่เป็นชื่อหุ้น (สมมติว่าชื่อหุ้นเป็นตัวพิมพ์ใหญ่และอยู่ตอนต้น)
    # รองรับรูปแบบ: SYMBOL_... หรือ SYMBOL-... หรือ SYMBOL...
    match = re.match(r'^([A-Z]+)[_-]?', filename_without_ext)
    if match:
        return match.group(1)
    else:
        # ถ้าไม่พบรูปแบบที่คาดหวัง ให้ใช้ 5 ตัวอักษรแรก (หรือทั้งหมดถ้าสั้นกว่า)
        return filename_without_ext[:5].upper()

def generate_html(image_urls):
    # จัดกลุ่มรูปภาพตามหุ้น
    stock_images = {}
    for url in image_urls:
        stock_symbol = extract_stock_symbol(url)
        if stock_symbol not in stock_images:
            stock_images[stock_symbol] = []
        stock_images[stock_symbol].append(url)
    
    # สร้างส่วนรายชื่อหุ้น
    stock_list_html = """
        <div class="stock-list">
            <div class="stock-item active" data-stock="all">All Stocks</div>
    """
    for stock in sorted(stock_images.keys()):
        stock_list_html += f'<div class="stock-item" data-stock="{stock}">{stock}</div>\n'
    stock_list_html += '</div>'
    
    # สร้างส่วน gallery
    gallery_html = '<div class="gallery">\n'
    for stock, urls in stock_images.items():
        for i, url in enumerate(urls):
            gallery_html += f'<div class="gallery-item" data-stock="{stock}">\n'
            gallery_html += f'<img src="{url}" alt="{stock} Chart {i+1}" onclick="openModal(this.src)">\n'
            gallery_html += '</div>\n'
    gallery_html += '</div>'
    ...
    <head>
        ...
        <title>Stock Charts Gallery</title>
        <style>
            body {{
                margin: 0;
        ...
        <script>
            ...
            function filterStocks(stock) {{
                ...
                galleryItems.forEach(item => {{
                    if (stock === 'all' || item.getAttribute('data-stock') === stock) {{
                        item.style.display = 'flex';
                        
                        // Scroll to the first item of the selected stock
                        if (stock !== 'all') {{
                            const firstItem = document.querySelector(`.gallery-item[data-stock="${{stock}}"]`);
                            if (firstItem) {{
